...
def Decision(Round, n, ki,MatsuiRoundIndex, MatsuiCount, flag):
    ...
    countclause = CountClausesInSequentialEncoding(kNI, kBN, countclause)
    # print(countclause)
    ...
    for r in range(0, Round):
        ...
        kC.append(kp1[r][0])
        kC.append(kp2[r][0])
    GenSequentialEncoding(kC, kui, kNI, kBN , file)
    ...
    file.close()
    # Call solver cadical
    order = "/home/zhangyh/Software/cadical/build/cadical " + "Problem-Round" + str(Round) + "-Probability" + str(ki)+ ".cnf > Round" + str(Round) + "-Probability" + str(ki) + "-solution.out"
    os.system(order)
    # Extracting results
